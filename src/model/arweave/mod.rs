...
pub struct KVChainArweaveDocument {
    ...
    pub previous_arweave_id: String,
}    

impl KVChainArweaveDocument {

    pub async fn upload_to_arweave(mut self) -> Option<String> {
        // create the signer
        ...
        let wallet = arweave_rs::wallet::WalletInfoClient::new(arweave_url);
        let address = arweave_connect.get_wallet_address().unwrap();
        // self.previous_arweave_id = wallet.last_tx_id(&address).await.unwrap();
